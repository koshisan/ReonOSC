...
///
/// Lifecycle:
///   var reader = new PfSignalReader();
///   reader.Start();                  // best-effort; logs and disables itself
///                                    // if SteamVR/OpenVR isn't available
///   reader.SignalChanged += ...;     // fires when sampled colour changes
///   reader.Stop();                   // safe to call repeatedly
/// </summary>
public sealed class PfSignalReader : IDisposable
{
    ...
    public void Start()
    {
        ...
        Logf($"PF signal reader started ({1000 / PollInterval.TotalMilliseconds:0} Hz poll).");
    }
    ...
    private bool TryInit()
    {
        try
        {
            ...
            // D3D11 device for the staging copy. Hardware adapter, no rendering.
    ...
    private bool TrySample(out PfSignalSample sample)
    {
        ...
        // Pull the latest mirror texture for the left eye. Right eye is roughly
        // identical for the static signal area — left is fine. The OpenVR API
        // returns the SRV via a ref IntPtr, AddRef'd; release with
        // ReleaseMirrorTextureD3D11 to balance.
        ...
        try
        {
            // QueryInterface for the SRV adds a ref so Vortice's Dispose has
            // something to release without stepping on OpenVR's
    ...
    private static PfSignalSample AveragePixel(MappedSubresource map, Format format)
    {
        ...
        unsafe
        {
            ...
            for (int y = 0; y < SampleRegion; y++)
            {
                ...
                {
                    // The compositor format is normally R8G8B8A8_UNorm or
                    // B8G8R8A8_UNorm. Handle both.
